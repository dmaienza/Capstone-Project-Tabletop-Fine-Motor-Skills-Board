...
#define DEADZONE 200 // Define the deadzone threshold
...
//    Serial.print("   Angle: " + String(angle) + " rad, " + String(angle * 180 / PI) + " deg");
    ...
    if (magnitude < DEADZONE) {
        // Scale saturation from 255 (edge of deadzone) to 0 (center)
        sat = map(magnitude, 0, DEADZONE, 0, 255);
    } else {
        // Set saturation to full if outside deadzone
        sat = 255;
    }
...
//    Serial.print("   Hue: "); Serial.print(hue);
    Serial.print("   Saturation: "); Serial.println(sat);
...
//        Serial.println("LED Current Brightness [" + String(i) + "]: " + String(currentBrightness[i]));
        ...
        leds[i] = CHSV(hue, sat, currentBrightness[i]);
    }
